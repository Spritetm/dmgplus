...
/home/pi/spi_cart_reader/spi_cart_reader -check
res=$?
...
if [ $res = 0 ]; then
	...
	mount / -o remount,rw
	HOME=/root /home/pi/gnuboy/sdlgnuboy --bind w "+b" --bind x "+a" --bind enter "+start" --bind space "+select" /dev/null
	exec /sbin/init
elif [ $res = 2 ]; then
	name=`/home/pi/spi_cart_reader/spi_cart_reader -getname`
	if [ $name = doom ]; then
		HOME=/root /usr/games/doom -config /root/prboom.cfg
	elif [ $name = smw ]; then
		cd /root
		HOME=/root /home/pi/pisnes/snes9x "/home/pi/Super Mario World (USA).sfc"
	elif [ $name = sonic ]; then
		HOME=/root /home/pi/osmose-rpi/osmose "/home/pi/Sonic The Hedgehog (USA, Europe).sms" -nn2x -inifile /home/pi/osmose-rpi/osmose.ini
	fi
	exec /sbin/init
else
	exec /sbin/init
